...
import org.onetwo.core.weak.db.query.DBQuery;
...
public class DefaultDatabaseManage implements DatabaseManager {
	...
	public void initDatabaseManager() {
		try {
			String cls = this.dbconfig.getProperty("jdbc.driverClass");
	...
	public String getDatabaseName(){
		...
		try {
			...
			if(isMySQL())
				...
				DBQuery dbquery = new DBQuery(new DBConnecton(conn));
				dbname = (String)dbquery.unique("select distinct TABLESPACE_NAME from tabs", null);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally{
	...
	public void setDbconfigPath(String dbconfig) {
		setDbconfig(new PropConfig(dbconfig, false).getConfig());
	}
	...
	public String getDataBaseType() {
		return getDbconfig().getProperty(DBKeys.type);
	}

	public String getJdbcDriver() {
		return getDbconfig().getProperty(DBKeys.jdbc_driver);
	}
